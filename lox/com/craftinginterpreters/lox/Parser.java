...
class Parser {
    ...
    private int current = 0; // next token waiting to be used
    ...
    private Stmt declaration() {
        try {
            ...
            synchronize(); // go to the next statement/declaration
            return null;
        }
    }
    ...
    private Stmt forStatement() {
        ...
        if (increment != null) {
            // replace body with block which contains original body + increment expr stmt
    ...
    private Stmt.Function function(String kind) {
        ...
        if(!check(RIGHT_PAREN)) { // handle  0-parameter case
            do {
                if (params.size() >= 8) {
    ...
    private Expr assignment() {
        ...
        if (match(EQUAL)) {
            ...
            if (expr instanceof Expr.Variable) {
                ...
                return new Expr.Set(get.object, get.name, value); // get expression -> corresponding set expression
            }
    ...
    private Expr equality() {
        ...
        while(match(BANG_EQUAL, EQUAL_EQUAL)) {
            ...
            // combine operator/operands into Expr.Binary syntax tree node
    ...
    private Expr finishCall(Expr callee) {
        ...
        if(!check(RIGHT_PAREN)) { // handle zero-argument case
            do {
                if(args.size() >= 8) {
    ...
    private Expr call() {
        ...
        while(true) {
            if (match(LEFT_PAREN)) {
                expr = finishCall(expr); // expr becomes Expr.Call (handles e.g. getFn()()())
            }
            else if (match(DOT)) {
